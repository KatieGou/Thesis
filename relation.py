...
class RetrievalDataset(Dataset):
    def __init__(self, tokenizer, args, split='train', is_train=True) -> None:
        super().__init__()
    ...
    def get_label(self, index): # index: [0, len(dataset)]
        img_idx, cap_idx=self.get_image_caption_index(index)
        if self.img_keys[img_idx]==cap_idx[0]:
            return 1
        else:
            return 0
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, cls_token_segment_id=0, pad_token_segment_id=0, sequence_a_segment_id=0, sequence_b_segment_id=1):
        ...
        if text_b:
            ...
            if len(tokens_b)>self.max_seq_len - seq_a_len - 1:
                tokens_b = tokens_b[: self.max_seq_len - seq_a_len - 1]
        ...
        for step, (_, batch) in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    logger.info(
                        "Epoch: {}, global_step: {}, lr: {:.6f}, loss: {:.4f} ({:.4f}), score: {:.4f} ({:.4f})".format(
...
def restore_training_settings(args):
    ...
    train_args = torch.load(op.join(args.eval_model_dir, 'training_args.bin')) # Loads an object saved with torch.save from a file
    override_params = ['do_lower_case', 'img_feature_type', 'max_seq_length', 'max_img_seq_length', 'add_od_labels', 'od_label_type', 'use_img_layernorm', 'img_layer_norm_eps']
...
def main():
    ...
    parser.add_argument("--data_dir", default='data/coco/train', type=str, required=False, help="The input data dir with all required files.")
    parser.add_argument("--img_feat_file", default='data/coco/train/features.tsv', type=str, required=False, help="The absolute address of the image feature file.")
    ...
    parser.add_argument("--max_seq_length", default=70, type=int, help="The maximum total input sequence length after tokenization.")
    ...
    parser.add_argument("--eval_img_keys_file", default='', type=str, help="image key tsv to select a subset of images for evaluation.")
    parser.add_argument("--eval_caption_index_file", default='', type=str, help="index of a list of (img_key, cap_idx) for each image.")
    ...
    parser.add_argument("--att_mask_type", default='CLR', type=str, help="attention mask type, support ['CL', 'CR', 'LR', 'CLR']")
