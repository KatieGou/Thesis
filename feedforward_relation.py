...
#     def __init__(self, tokenizer, args, split: str, is_train: bool, config):
...
#         self.img_embeddings=nn.Linear(args.img_feature_dim, config.hidden_size)
#         self.args=args
...
#         input_ids=self.input_embeddings(input_ids) # (seq_len, hidden_size)
...
#         img_feat=self.img_embeddings(img_feat) # (img_seq_len, hidden_size)
        
#         return torch.cat([input_ids, img_feat], dim=0).detach() # (seq_len+img_seq_len, hidden_size)
...
#     tokenizer_class, config_class = BertTokenizer, BertConfig
#     if args.do_train:
#         tokenizer=tokenizer_class.from_pretrained(args.model_name_or_path, do_lower_case=args.do_lower_case)
#         config=config_class.from_pretrained(args.model_name_or_path)
#         args.input_size=(args.max_seq_length+args.max_img_seq_length)*config.hidden_size
#         model=Baseline(args).to(args.device)
#         logger.info('Training parameters: {}'.format(args))
#         train_dataset=RetrievalDataset(tokenizer=tokenizer, args=args, split='train', is_train=True, config=config)
#         if args.evaluate_during_training:
#             val_dataset=RetrievalDataset(tokenizer=tokenizer, args=args, split='val', is_train=False, config=config)
#         else:
#             val_dataset=None
...
#         test_dataset=RetrievalDataset(tokenizer=tokenizer, args=args, split='test', is_train=False, config=config)
